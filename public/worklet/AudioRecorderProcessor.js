// export type AudioRecorderProcessorChunk = Array<Array<Float32Array>> // array of channels. Each channel have Float32Array values. TODO:
// 
// 
...
export default class AudioRecorderProcessor extends AudioWorkletProcessor {
  ...
  process (inputs, outputs, parameters) {
    if (parameters.isRecording[0] === 0) { //we can enable or disable recording porcess by set isRecordin parametr 0 or 1
      return true; // false will stop the porcess
    }
    if(inputs[0].length !== CHANNEL_COUNT) { // when playback is stopped process continues receive rendering quantums (blocks)
      return true;
    }
    const chunk = [[],[]];
    const channel1 = 0;
    ...
    for (let t = 0; t < inputs[0][channel1].length; t += 1) { //TODO: remake
