...
export class Context extends BaseContext {
	...
	async addAudioWorkletModule(url: string): Promise<void> {
		...
		const result = await Promise.race([this._workletPromise, Promise.resolve("pending")])
		if (result === "pending") {
			console.warn("The previous worklet is still loading. Be careful, something might break");
		}
