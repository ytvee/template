  ...
  <div class="work-area-settings">
    ...
    <div v-if="isExpanded" class="settings-holder">
      <div class="field">
        ...
        <input id="myRange" v-model="timeScaleSliderValue" type="range" min="1" max="100" class="slider" />
        <div class="input-holder">
          <input v-model="timeScaleSliderValue" type="text" />
        </div>
      </div>
      <div class="field">
        <span class="field-key"> noteScale, notes/px: </span>
        <input id="myRange" v-model="noteScaleSliderValue" type="range" min="0.01" max="1" step="0.01" class="slider" />
        <div class="input-holder">
          <input v-model="noteScaleSliderValue" type="text" />
        </div>
      </div>
      <div class="field">
        <span class="field-key"> baseRiskTimeStep, seconds: </span>
        <input id="myRange" v-model="baseRiskTimeStepSliderValue" type="range" min="100" max="10000" class="slider" />
        <div class="input-holder">
...
export default {
  ...
  data() {
    return {
      ...
      noteScaleSliderValue: this.$props.navigationModel.noteScale,
  ...
  watch: {
    ...
    noteScaleSliderValue: {
      handler(newValue) {
        const newValueNumber = Number(newValue);
        if (newValueNumber > 0) {
          this.$eventBus.emit("work-area-settings-note-scale-update", Number(newValue));
        }
      },
    },
