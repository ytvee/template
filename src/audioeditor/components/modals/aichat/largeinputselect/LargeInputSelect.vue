<template>
  <div class="default-input-select" :class="{ expanded: dropDownShowed }" @click="toggleDropDown()">
    <div class="input-select-dropdown-trigger-container">
      <span class="input-select-label">{{ triggerLabel }}</span>
      <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg"
           :class="{ expandedbottom: dropDownShowed && !isChatActive, expandedup: dropDownShowed && isChatActive }"
      >
        <path
    ...
    <transition name="input-select-dropdown-transition">
      ...
      >
        <div
          v-for="column in options" :key="column.id"
          class="input-select-options-wrapper">
          <div
            v-for="option in column" :key="option.id"
            class="option-container"
            @click="variant === 'multi' ? multiSelectOptionHandle(option) : singleSelectOptionHandle(option)"
          >
            <span>{{ option }}</span>
            <svg
              v-show="selectedOption === option"
              width="10"
              height="8"
              viewBox="0 0 10 8"
              fill="none"
...
export default {
  ...
  data() {
    return {
      ...
      // selectedOptions: this.variant === "multi" ? [] : "",
    };
  },
  methods: {
    multiSelectOptionHandle(optionLabel) {
      if (this.variant !== "multi") return;

      if (!this.selectedOptions.includes(optionLabel)) {
        this.selectedOptions.push(optionLabel);
        this.$nextTick(() => {
          this.$emit("add-message-meta-data", { key: "instruments", value: this.selectedOptions });
        });
        return;
      }
    ...
    singleSelectOptionHandle(optionLabel) {
      if (this.variant !== "single") return;

      if (!(this.selectedOptions === optionLabel)) {
        this.selectedOptions = optionLabel;
        this.$nextTick(() => {
          this.$emit("add-message-meta-data", { key: this.triggerLabel.toLowerCase(), value: this.selectedOptions });
        });
        return;
      }
      this.selectedOptions = "";
      this.$nextTick(() => {
        this.$emit("add-message-meta-data", { key: this.triggerLabel.toLowerCase(), value: "" });
      });
    },
    toggleDropDown() {
...
.input-select-container.reverse {
  background: #373737;
  border-radius: 15px;
  box-shadow: 0px 22px 32px 0px #0000004D;
